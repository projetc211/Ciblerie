...
*/
// AJOUT EN GLOBAL (en haut du fichier, après les includes OU juste avant les fonctions LJ)
...
void trtPartieEnCoursLJ()
{
    ...
    if (statusCible10 == LOW && !cible10Traitee){
      ...
      Serial.println("📤 Envoi à ESP32: " + message);
      cible10Traitee = true;
    } else if (statusCible10 == HIGH) {
      cible10Traitee = false;
    }
    else {
      killer[joueurEnCours] = 0;
      }
    // Si aucune cible touchée, on ne fait rien de plus ici (pas de killer affecté inutilement)
    SMRAZ();        
    GererInterruptionLJ();
}
