...
class _ScoresScreenState extends State<ScoresScreen> with TickerProviderStateMixin {
  ...
  bool attente = true; // true = bouton cumul à 0.000, false = afficher le vrai cumul
  bool showFinGameOverlay = false;
  bool hasNavigatedToClassement = false;

  // Blink pour joueur courant
  bool isBlinking = false;

  // Chronomètre global - notifiers
  ...
  void initState() {
    ...
    attente = true; // Toujours commencer en attente
    ...
    _messageSubscription = ws.messages.listen((message) {
      ...
      if (type == "game_status") {
        ...
        if (wsMessage == "START") {
          ...
          //         _gameEndedNotifier.value = false;
        }
  ...
  void _showFinGameOverlay() async {
    ...
    if (mounted && !hasNavigatedToClassement) {
      ...
      // RESET SCORES ici juste avant d'aller sur ClassementScreen
      Provider.of<WebSocketService>(context, listen: false).resetScores();
  ...
  Widget build(BuildContext context) {
    ...
    // Récupérer les pseudos personnalisés depuis le service
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          // Cumul synchronisé sur la ValueNotifier du service pour éviter tout décalage
