...
void Penalite()
{
  ...
  if(!coursesFinies && coursesCommencees) {
    ...
    if (!tempsGenere) {
      tempsJeu = tempsManche;  // Utilise le temps du mode choisi
      Period1 = tempsJeu * 1000;
      Period2 = tempsJeu * 1000;
      dernierCountdownEnvoye = tempsJeu;
      tempsGenere = true;
      // Affichage couleur code
      if ((int)tempsJeu == 1) fill_solid(leds, NUM_LEDS, CRGB::Red); // 🔴 Rouge
      else if ((int)tempsJeu >= 2 && (int)tempsJeu <= 5.0) {
    ...
    // Éteindre LEDs après 0.9s
    if (ledsAllumees && millis() - ledsAllumeesMillis >= 500) {
      FastLED.clear(); FastLED.show();
      ledsAllumees = false;
    }
    // Changement de couleur toutes les secondes si entre 2 et 5s
    if ((int)tempsJeu >= 2 && (int)tempsJeu <= 5.0) {
      int index = (int)tempsJeu - 2;
